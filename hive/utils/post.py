...
def _score_with_weighted_median_rshares(active_votes, created_timestamp, timescale=480000):
    """Calculate trending/hot score with weighted median rshares."""
    if active_votes:
        rshares = [int(vote['rshares']) for vote in active_votes]
        length = len(rshares)
        median_rshare = sorted(rshares)[length // 2]
        weighted_median_rshare = median_rshare * math.log10(length) # Weighted median
    else:
